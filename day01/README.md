...
5. Compile today's kernel with `--keep` and look at the generated `.ptx` file. Then compile once targeting a newer architecture than your GPU actually is (e.g. `-arch=sm_90` on a Turing card) and observe the runtime error. Try `-arch=sm_50` too: on CUDA 13 or newer that now fails at *compile* time rather than run time, because Maxwell/Pascal/Volta support was removed from the toolkit — a different failure mode worth being able to tell apart.
